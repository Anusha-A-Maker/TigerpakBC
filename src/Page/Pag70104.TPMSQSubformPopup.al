...
page 70104 "Master Sales Quote Lines"
{
    ApplicationArea = All;
    Caption = 'Master Sales Quote Lines';
    PageType = ListPart;
    SourceTable = "Sales Line";

    SourceTableView = SORTING("No.", "Line No.") ORDER(Ascending) WHERE("Document Type" = CONST(Quote));
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field("TP Unit Cost"; Rec."TP Unit Cost")
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies the unit cost of the item on the line. with added Inflated % respect to location';
                    Visible = true;
                    Editable = false;
                    StyleExpr = MyStyleVar;
                }
            ...
            group("F&unctions")
            {
                ...
                action("Make Quote")
                {
                    ...
                    var
                        ...
                        LineNumber: Integer;
                    begin
                        UpdatePrice(); //Sales Price list
                        LineNumber := 0;
                        SalesLne.Reset();
                        SalesLne.SetRange("Document Type", SalesLne."Document Type"::Quote);
                        SalesLne.SetRange("Document No.", Rec."Document No.");
                        SalesLne.SetRange("Include in Make Quote", true);
                        if SalesLne.findset then begin
                            SalesHdr.Init;
                            SalesHdr.SetHideValidationDialog(true);
                            SalesHdr.Validate("Document Type", SalesLne."Document Type"::Quote);
                            SalesHdr."No." := '';
                            SalesHdr.Validate("Sell-to Customer No.", SalesLne."Sell-to Customer No.");
                            SalesHdr."Master SQ No." := SalesLne."Document No.";
                            SalesHdr.Insert(true);
                            repeat

                                LineNumber := LineNumber + 10000;
                                salesLne2.Init;
                                salesLne2.Validate("Document Type", SalesLne."Document Type");
                                salesLne2.Validate("Document No.", SalesHdr."No.");
                                salesLne2.Validate("Line No.", LineNumber);
                                salesLne2.Validate("Sell-to Customer No.", SalesLne."Sell-to Customer No.");
                                salesLne2.Insert(true);
                                salesLne2.Validate(Type, SalesLne.Type);
                                salesLne2.Validate("No.", SalesLne."No.");
                                salesLne2.Validate("VAT Prod. Posting Group", SalesLne."VAT Prod. Posting Group");
                                salesLne2.validate(Description, SalesLne.Description);
                                salesLne2.Validate("Description 2", SalesLne."Description 2");
                                salesLne2.Validate(Quantity, SalesLne.Quantity);
                                salesLne2.Validate("Unit Price", SalesLne."Unit Price");
                                salesLne2.Validate("Line Amount", SalesLne."Line Amount");
                                salesLne2.Validate("Shortcut Dimension 1 Code", SalesLne."Shortcut Dimension 1 Code");
                                salesLne2.Validate("Shortcut Dimension 2 Code", SalesLne."Shortcut Dimension 2 Code");
                                salesLne2.Modify(true);
                                SalesLne.Validate("Include in Make Quote", false);
                                SalesLne.Modify();
                            until SalesLne.Next() = 0;
                            // Message('Quote No. %1 is created', SalesHdr."No.");
                            if confirm('Sales Quote No. %1 is created, Do you want to Open', True, SalesHdr."No.") then begin
                ...
                action("Make Order")
                {
                    ...
                    begin
                        ...
                        if SalesLne.findset then begin
                            SalesHdr.Init;
                            SalesHdr.SetHideValidationDialog(true);
                            SalesHdr.Validate("Document Type", SalesLne."Document Type"::Order);
                            SalesHdr.Insert(true);
                            //SalesHdr."No." := '';
                            SalesHdr.Validate("Sell-to Customer No.", SalesLne."Sell-to Customer No.");
                            SalesHdr."Master SQ No." := SalesLne."Document No.";
                            ...
                            repeat
                                ...
                                LineNumber := LineNumber + 10000;
                                salesLne2.Init;
                                salesLne2.Validate("Document Type", salesLne2."Document Type"::Order);
                                salesLne2.Validate("Document No.", SalesHdr."No.");
                                salesLne2.Validate("Line No.", LineNumber);
                                salesLne2.Validate("Sell-to Customer No.", SalesLne."Sell-to Customer No.");
                                salesLne2.Insert(true);
                                ...
                                SalesLne.Modify();
                            until SalesLne.Next() = 0;
                            //Message('Order No. %1 is created', SalesHdr."No.");
                            if confirm('Sales Order No. %1 is created, Do you want to Open', True, SalesHdr."No.") then begin
    ...
    begin
        //AR GetTotalSalesHeader();
        //AR CalculateTotals();
        //AR UpdateEditableOnRow();
        UpdateTypeText();
        //AR SetItemChargeFieldsStyle();
        //AR   UpdateStyle();
    end;
    ...
    var
        Item: Record Item;
    begin
        //AR Rec.ShowShortcutDimCode(ShortcutDimCode);
        //AR UpdateEditableOnRow();
        UpdateTypeText();
        //AR  SetItemChargeFieldsStyle();
        //AR if Rec."Variant Code" = '' then
        //AR             VariantCodeMandatory := Item.IsVariantMandatory(Rec.Type = Rec.Type::Item, Rec."No.");
        //AR   UpdateStyle();
    end;
    ...
    begin
        ...
        // // Filter by Document No. passed from the subform
    ...
    begin
        CurrPage.Update();
        SalesHeader.Get(Rec."Document Type", Rec."Document No.");
        //AR  SalesCalcDiscByType.ApplyDefaultInvoiceDiscount(TotalSalesHeader."Invoice Discount Amount", SalesHeader);
    end;
    ...
    var
        ...
        ItemRec: Record Item;
    begin
        IF Not ItemRec."Exclude Item from Price Check" and (Rec.Type = rec.Type::Item) then begin
            SKU.Reset();
                    ...
                    UnitPriceLCY :=
                      ROUND(
                        CurrExchRate.ExchangeAmtFCYToLCY(
                          WORKDATE, Rec."Currency Code",
                          Rec."Unit Price", CurrExchRate.ExchangeRate(WORKDATE, Rec."Currency Code")),
                        Currency."Amount Rounding Precision")
                ELSE
                    UnitPriceLCY :=
                      ROUND(Rec."Unit Price", Currency."Amount Rounding Precision");


                IF UnitPriceLCY < SKU."Unit Cost" then begin
                    MyStyleVar := 'Unfavorable';
                end else begin
                    MyStyleVar := 'Normal';
                end;
    ...
    var
        ...
        SalesSetup: Record "Sales & Receivables Setup";
        SalesListLine: Record "Price List Line";
        SaleslistheaderRec: Record "Price List Header";
        PriceListManagement: Codeunit "Price List Management";
        SalesLineRec: Record "Sales Line";
        SalesListLine2: Record "Price List Line";
    begin
        SalesSetup.Get();
        if SalesSetup."Default Price List Code" <> '' then begin
            SaleslistheaderRec.get(SalesSetup."Default Price List Code");
            SalesLineRec.reset;
            saleslinerec.SetRange("Document Type", Rec."Document Type");
            SalesLineRec.setrange("Document No.", Rec."Document No.");
            SalesLineRec.SetRange(Type, Rec.Type::Item);
            //AR commented based on justin email
            //SalesLineRec.SetRange("Include in Make Quote", true);
            //CurrPage.SetSelectionFilter(SalesLineRec);
            if SalesLineRec.FindFirst() then
                repeat
                    SalesListLine.SetRange("Price List Code", SalesSetup."Default Price List Code");
                    SalesListLine.SetRange("Source Type", SalesListLine."Source Type"::Customer);
                    salesListLine.SetRange("Assign-to No.", SalesLineRec."Sell-to Customer No.");
                    SalesListLine.SetRange("Unit of Measure Code", SalesLineRec."Unit of Measure Code");
                    SalesListLine.SetRange("Asset Type", SalesListLine."Asset Type"::Item);
                    SalesListLine.SetRange("Asset No.", SalesLineRec."No.");
                    SalesListLine.SetRange("Minimum Quantity", SalesLineRec.Quantity);
                    if SalesListLine.FindFirst() then begin
                        SalesListLine."Unit Price" := SalesLineRec."Unit Price";
                        SalesListLine."Minimum Quantity" := SalesLineRec.Quantity;
                        SalesListLine."Starting Date" := today();
                        SalesListLine."Ending Date" := CalcDate('<+6M>', SalesListLine."Starting Date");
                        SalesListLine.Modify();
                    end else begin
                        SalesListLine.Init();
                        SalesListLine2.reset;
                        SalesListLine2.SetRange("Price List Code", SalesSetup."Default Price List Code");
                        // SalesListLine2.SetRange("Line No.", 5000, 19999);
                        if SalesListLine2.findlast Then
                            SalesListLine."Line No." := SalesListLine2."Line No." + 10
                        else
                            SalesListLine."Line No." := 10;
                        SalesListLine.Validate("Price List Code", SalesSetup."Default Price List Code");
                        SalesListLine.Validate("Source Type", SalesListLine."Source Type"::Customer);
                        SalesListLine.Validate("Source No.", SalesLineRec."Sell-to Customer No.");
                        SalesListLine.Validate("Assign-to No.", SalesLineRec."Sell-to Customer No.");
                        SalesListLine.Validate("Asset Type", SalesListLine."Asset Type"::Item);
                        SalesListLine.Validate("Product No.", SalesLineRec."No.");
                        SalesListLine."Minimum Quantity" := SalesLineRec.Quantity;
                        SalesListLine."Asset No." := SalesLineRec."No.";
                        SalesListLine."Description" := SalesLineRec."Description";
                        SalesListLine."Unit of Measure Code" := SalesLineRec."Unit of Measure Code";
                        SalesListLine."Unit of Measure Code Lookup" := SalesLineRec."Unit of Measure Code";
                        SalesListLine."Unit Price" := SalesLineRec."Unit Price";
                        SalesListLine."Starting Date" := today();
                        SalesListLine."Ending Date" := CalcDate('<+6M>', SalesListLine."Starting Date");
                        SalesListLine.Insert();
                    end;
                until SalesLineRec.Next() = 0;

            PriceListManagement.ActivateDraftLines(SaleslistheaderRec);//Activate price

            Message('Price List is updated');
