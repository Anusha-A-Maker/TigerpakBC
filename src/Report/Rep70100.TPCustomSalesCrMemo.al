...
report 70100 "TP Custom Sales Cr. Memo"
{
    ApplicationArea = All;
    Caption = 'TP Custom Sales Cr. Memo';
    UsageCategory = ReportsAndAnalysis;
    Permissions = TableData "Sales Shipment Buffer" = rimd;
    DefaultRenderingLayout = "CustomSalesCreditMemo.rdl";
    ...
    dataset
    {
        dataitem(SalesCrMemoHeader; "Sales Cr.Memo Header")
        {
            ...
            column(CompanyABN_Lbl; CompanyInfo.fieldcaption(companyinfo.abn))
            {
            }
            ...
            column(CompanyBankBranchNo_Lbl; CompanyBankAccount.FieldCaption(CompanyBankAccount."Bank Branch No."))
            {
            }
            ...
            column(CompanyIBAN_Lbl; CompanyBankAccount.FieldCaption(CompanyBankAccount.IBAN))
            {
            }
            column(CompanySWIFT; CompanyBankAccount."SWIFT Code")
            {
            }
            column(CompanySWIFT_Lbl; CompanyBankAccount.FieldCaption(CompanyBankAccount."SWIFT Code"))
            {
            }
            ...
            dataitem(Line; "Sales Cr.Memo Line")
            {
                ...
                column(Type_Line; Format(Type, 0, 2))
                {
                }
                ...
                column(LineAmount_Line2; "Line Amount")
                {
                    AutoFormatExpression = GetCurrencyCode();
                    AutoFormatType = 1;
                }
                ...
                column(GSTAmountTxt; GSTAmountTxt)
                {
                ...
                begin
                    ...
                    if "No." = '' then
                        ...
                        GSTAmountTxt := Format(Round("Amount Including VAT" - "Line Amount", 0.01, '=')) + '.00';
            ...
            dataitem(WorkDescriptionLines; Integer)
            {
                ...
                begin
                    if not ShowWorkDescription then
                        CurrReport.Break();
                    SalesCrMemoHeader."Work Description".CreateInStream(WorkDescriptionInstream, TEXTENCODING::UTF8);
                end;
            }
            ...
            dataitem(VATClauseLine; "VAT Amount Line")
            {
                ...
                begin
                    ...
                    if not VATClause.Get("VAT Clause Code") then
                        CurrReport.Skip();
                    VATClauseText := VATClause.GetDescriptionText(SalesCrMemoHeader);
                end;
            }
            ...
            begin
                ...
                FormatAddressFields(SalesCrMemoHeader);
                FormatDocumentFields(SalesCrMemoHeader);
                if SellToContact.Get("Sell-to Contact No.") then;
                if BillToContact.Get("Bill-to Contact No.") then;
                Clear(CustomerPONo);
                if SalesInvoiceHeader.get("Adjustment Applies-to") then
                    CustomerPONo := SalesCrMemoHeader."External Document No.";

                if not Cust.Get("Bill-to Customer No.") then
                    Clear(Cust);

                if not CompanyBankAccount.Get(SalesCrMemoHeader."Company Bank Account Code") then
    ...
    rendering
    {
        layout("CustomSalesCreditMemo.rdl")
        {
            Type = RDLC;
            LayoutFile = './src/Layouts/CustomSalesCreditMemo.rdl';
            Caption = 'Custom Sales Credit Memo (RDL)';
            Summary = 'The Custom Sales Credit Memo (RDL) provides a detailed layout.';
        }
    }
                ...
                repeat
                    if SalesCrMemoHeader."Bill-to Contact No." <> '' then
                        SegManagement.LogDocument(
                          6, SalesCrMemoHeader."No.", 0, 0, DATABASE::Contact, SalesCrMemoHeader."Bill-to Contact No.", SalesCrMemoHeader."Salesperson Code",
                          SalesCrMemoHeader."Campaign No.", SalesCrMemoHeader."Posting Description", '')
                    else
                        SegManagement.LogDocument(
                          6, SalesCrMemoHeader."No.", 0, 0, DATABASE::Customer, SalesCrMemoHeader."Bill-to Customer No.", SalesCrMemoHeader."Salesperson Code",
                          SalesCrMemoHeader."Campaign No.", SalesCrMemoHeader."Posting Description", '');
    ...
    trigger OnPreReport()
    begin
        // if SalesCrMemoHeader.GetFilters = '' then
    ...
    var
        ...
        SalesCreditMemoLbl: Label 'Credit Note';
    ...
    begin
        ...
        ShipmentLine.GetLinesForSalesCreditMemoLine(Line, SalesCrMemoHeader);
    ...
    begin
        ...
        AppliesToText :=
              FormatDocument.SetText(SalesCrMemoHeader."Applies-to Doc. No." <> '', StrSubstNo('%1 %2', '', SalesCrMemoHeader."Applies-to Doc. No."));
    end;
